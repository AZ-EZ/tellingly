...
const subscriptions = [
  {
    ...
    screenshot: "app-store/subscription-screenshots/tellingly-plus-monthly-review.png",
  },
  {
    id: "6783584539",
    productId: "app.tellingly.plus.annual",
    screenshot: "app-store/subscription-screenshots/tellingly-plus-annual-review.png",
  },
];

const reviewNote =
  "Tellingly Plus unlocks the full answer archive, trait trends, unlimited compatibility links, comparison history, and extra question packs. In the fixed 1.0 build 2, trigger the paywall by answering today's question, finishing the round, creating one free compatibility link, then tapping Compare again. The paywall includes monthly and annual plans plus a visible Restore Purchases control.";
...
for (const subscription of subscriptions) {
  const bytes = fs.readFileSync(subscription.screenshot);
  const fileName = subscription.screenshot.split("/").pop();
  const checksum = crypto.createHash("md5").update(bytes).digest("hex");

  await api("PATCH", `/v1/subscriptions/${subscription.id}`, {
    data: {
      type: "subscriptions",
      id: subscription.id,
      attributes: {
  ...
  const existing = await api("GET", `/v1/subscriptions/${subscription.id}/appStoreReviewScreenshot`);
  if (existing.data?.id) {
  ...
  const updated = await api("PATCH", `/v1/subscriptionAppStoreReviewScreenshots/${created.data.id}`, {
    data: {
      type: "subscriptionAppStoreReviewScreenshots",
      id: created.data.id,
      attributes: {
